...
struct VoxelVertex final {
    constexpr static const float POS_EPSILON = 1.0f / 16.0f;
    constexpr static const float UV_EPSILON = 1.0f / 16.0f;
    uint32_t pack[2];
    VoxelVertex() = default;
...
inline VoxelVertex::VoxelVertex(const float3_t &pos, const float2_t &uv, uint16_t tex_id)
{
    ...
    pack[1] = (0x0000FFFF & glm::packUnorm4x8(float4_t(uv.x, uv.y, 0.0f, 0.0f) * UV_EPSILON)) | (static_cast<uint32_t>(tex_id) << 16);
}

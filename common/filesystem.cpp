...
bool fs::setRoot(const stdfs::path &path)
{
    const stdfs::path new_root = stdfs::current_path() / path;
    if(!stdfs::is_directory(new_root) && !stdfs::create_directories(new_root))
        return false;
    root_path = new_root;
    return true;
}
...
{
    for(const stdfs::path &it : mount_points) {
        stdfs::path full_path = root_path / it / path;
